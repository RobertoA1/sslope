...
`npm run report:research` consolida tablas CSV de comparación de modelos (incluidos ambos orígenes temporales), errores estratificados por lluvia y movimiento, bootstrap pareado por fecha, selección según validación, ablación, robustez, sensibilidad de malla y detección de episodios de desplazamiento alto. En 2024 la LSTM logra ligeramente menor MAE que el híbrido y en 2025 ocurre lo inverso; ninguno de los cuatro contrastes pareados excluye cero. El umbral de detección de episodios altos es el percentil 95 calculado únicamente en entrenamiento, no un umbral operativo de mina. Un manifiesto SHA-256 registra exactamente los artefactos fuente.
