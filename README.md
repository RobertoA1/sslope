...
La malla de análisis predeterminada es 30×20. `npm run validate:fem` genera `data/validation/ta01-fem-mesh-sensitivity.json`, compara refinamientos hasta 48×32 y verifica el ensamblaje global con soluciones analíticas afines de elasticidad lineal sin agua y con presión de poros uniforme de Biot. Los errores máximos son aproximadamente 1,62×10⁻¹¹ y 1,60×10⁻¹¹ m, respectivamente. Es una verificación del código y del signo de la carga mecánica de agua; no valida infiltración, parámetros TA-01 ni estabilidad geotécnica, y aún falta que el FEM propio supere un benchmark geotécnico externo.
...
El CSV usa una fila por escenario-hora y el manifiesto JSON registra semilla, procedencia y parámetros. La evaluación principal separa por **fecha de lluvia**: ningún escenario del mismo día cruza particiones. Adicionalmente, hay dos pruebas cronológicas con reentrenamiento independiente: prueba 2024 (se excluye 2025) y prueba 2025. Todas son semisintéticas, no validaciones de campo. La especificación científica y los comandos de reproducción están en [CASO_TA01_FEM.md](docs/CASO_TA01_FEM.md).

`npm run report:research` consolida tablas CSV de comparación de modelos (incluidos ambos orígenes temporales), errores y cobertura de intervalos estratificados por lluvia y movimiento, bootstrap pareado por fecha, selección según validación, ablación, robustez, sensibilidad de malla y detección de episodios de desplazamiento alto. En 2024 la LSTM logra ligeramente menor MAE que el híbrido y en 2025 ocurre lo inverso; ninguno de los cuatro contrastes pareados excluye cero. Los grupos de lluvia ≥5 mm/día tienen solo dos o tres fechas y muestran cobertura inferior al 95 % nominal. El umbral de detección de episodios altos es el percentil 95 calculado únicamente en entrenamiento, no un umbral operativo de mina. Un manifiesto SHA-256 registra exactamente los artefactos fuente.
...
| GET | `/api/research/chronological?horizon=6&testYear=2024` | comparar persistencia, ridge, LSTM e híbrido en el origen temporal 2024 o 2025 (`testYear`); por defecto usa 2025 |
...
Como caso real candidato separado de TA-01 se delimitó un [transecto de la pared noroeste del tajo Raúl Rojas](docs/CASO_RAUL_ROJAS_NW.md), con perfil SRTM histórico, lluvia regional del período de monitoreo 2009–2010 y desplazamientos acumulados publicados de 11 prismas. No existen todavía geometría contemporánea ni lecturas diarias georreferenciadas para entrenar o validar el pronóstico temporal en esa pared; la selección no habilita alertas operativas.
